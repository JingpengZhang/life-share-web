<template>
  ...
  <div class="flex flex-col gap-4">
    <AskItem v-for="item in data" :key="item.ID" :ask="item"></AskItem>
  </div>
...
<script>
import AskItem from "@/pages/user/components/AskItem.vue";
import { onMounted, ref } from "vue";
import { getUsersAsks, getUserAskCount } from "@/api/ask";
...
export default {
  ...
  setup(props) {
    ...
    const getData = () => {
      ...
      getUsersAsks(id, current_page.value).then((result) => {
