  ...
  <div class="border rounded-md bg-white py-3">
    ...
    <table class="text-sm w-full text-left mt-3">
      <tr class="bg-blue-100 h-10">
        ...
        <th class="pl-6 w-2/12">作者</th>
        <th class="pl-6 w-3/12">内容</th>
      ...
      <tr v-for="(item, index) in data" :key="item.ID" class="h-12 border-b">
        ...
        <td class="pl-6">{{ item.USER_USERNAME }}</td>
        <td class="pl-6">{{ item.CONTENT }}</td>
        <td class="pl-6">{{ item.USER_MAJOR }}</td>
        <td class="pl-6">{{ item.CREATE_TIME }}</td>
        <td class="pl-6 h-full">
          <div class="h-full flex items-center gap-2">
            <router-link :to="'/ask/'+item.ASK_ID">
...
import {getAnswersNum,getAllAnswer,deleteAskAnswer} from "../../../../../api/ask";
...
export default {
  ...
  setup() {
    ...
    const confirmDelete = () => {
      deleteAskAnswer(delete_id.value).then((result) => {
