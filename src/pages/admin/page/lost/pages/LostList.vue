  ...
  <div class="border rounded-md bg-white py-3">
    ...
    <table class="text-sm w-full text-left mt-3">
      <tr class="bg-blue-100 h-10">
        ...
        <th class="pl-6 w-1/12">配图</th>
        <th class="pl-6 w-1/12">发布者</th>
        <th class="pl-6 w-2/12">物品名称</th>
        <th class="pl-6 w-1/12">状态</th>
        <th class="pl-6 w-2/12">地点</th>
      ...
      <tr v-for="(item, index) in data" :key="item.ID" class="h-12 border-b">
        ...
        <td class="pl-6">
          <div class="w-16 h-16">
            <img :src="item.PIC" :alt="item.PIC" class="coverImg" />
          </div>
        </td>
        ...
        <td class="pl-6">
          <div :class="item.STATE_COLOR">
            {{ item.STATE }}
          </div>
        </td>
        <td class="pl-6">{{ item.POSITION }}</td>
        <td class="pl-6">{{ item.CREATE_TIME }}</td>
        <td class="pl-6 h-full">
          <div class="h-full flex items-center gap-2">
            <router-link :to="'/lost/'+item.ID">
...
import {getNumByType,getAllByType,deleteItem} from "@/api/lost.js";
...
export default {
  ...
  setup() {
    ...
    const changePage = (page) => {
      getAllByType('招领',page).then((result) => {
        ...
        for (let i = 0; i < preData.length; i++) {
          ...
          if(preData[i].STATE == '未解决'){
            preData[i].STATE_COLOR = 'text-red-500'
          }else{
            preData[i].STATE_COLOR = 'text-green-500'
          }
        }
